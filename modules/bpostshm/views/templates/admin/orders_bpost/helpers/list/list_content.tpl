...
{if $version >= 1.5 && $version < 1.6}
	...
	{foreach $list AS $index => $tr}
		<tr
		{if $position_identifier}id="tr_{$id_category|escape:'htmlall':'UTF-8'}_{$tr.$identifier|escape:'htmlall':'UTF-8'}_{if isset($tr.position['position'])}{$tr.position['position']}{else}0{/if}"{/if}
			...
			{foreach $fields_display AS $key => $params}
				{block name="open_td"}
					<td
						...
						{if (!isset($params.position) && !$no_link && !isset($params.remove_onclick))}
							onclick="document.location = '{$current_index}&{$identifier|escape:'htmlall':'UTF-8'}={$tr.$identifier|escape:'htmlall':'UTF-8'}{if $view}&view{else}&update{/if}{$table|escape:'htmlall':'UTF-8'}&token={$token|escape:'htmlall':'UTF-8'}'">
						{else}
						>
					{/if}
				{/block}
				{block name="td_content"}
					...
					{if isset($params.color) && isset($tr[$params.color])}
						<span class="color_field" style="background-color:{$tr[$params.color]|escape:'htmlall':'UTF-8'};color:{if Tools::getBrightness($tr[$params.color]|escape) < 128}white{else}#383838{/if}">
					{/if}
					{if isset($tr.$key)}
