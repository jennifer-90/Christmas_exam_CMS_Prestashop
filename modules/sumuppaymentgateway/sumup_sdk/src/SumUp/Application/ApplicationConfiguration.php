...
class ApplicationConfiguration implements ApplicationConfigurationInterface
{
     ...
     */
    const DEFAULT_SCOPES = array('payments', 'transactions.history', 'user.app-settings', 'user.profile_readonly');

    /**
     * The possible values for grant type.
     */
    const GRANT_TYPES = array('authorization_code', 'client_credentials', 'password');
     ...
     * @param array $scopes
     */
    protected function setScopes(array $scopes = array())
    {
    ...
    {
        $this->customHeaders = is_array($customHeaders) ? $customHeaders : array();
    }
}
